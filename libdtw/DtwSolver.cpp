...
namespace SignatureAuthLibrary
{
    ...
    bool DtwSolver::check(const vector< vector<int> >& firstSign, const vector< vector<int> >& secondSign)
    {
        ...
        int diagonalLength = std::max(N, M);
        ...
        int i = N-1;
        int j = M-1;
        vector<int> firstPoint;
        firstPoint.push_back(i);
        firstPoint.push_back(j);
        ...
        std::cout << "Optimal warping path: [";
        for (int i=0; i<path.size(); i++) {
            std::cout << "[" << path[i][0] << ", " << path[i][1] << "]";
        }
        std::cout << "]" << std::endl;

        float threshold = Configuration::instance()->getThreshold();

        float percent = diagonalLength / (float)path.size();

        std::cout << "diagonal: " << diagonalLength << std::endl;
